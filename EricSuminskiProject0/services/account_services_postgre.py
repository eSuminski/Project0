...
class AccountServicePostgre(AccountService):

    def __init__(self, account_dao: AccountDAOPostgres):
    ...
    def withdraw(self, client_id: int, account_id: int, withdraw: float):
        if float(withdraw) < 0:
            raise NegativeNumberException("Invalid withdraw amount (negative number)")
        c_account = self.get_client_account_by_id(client_id, account_id)
        if withdraw > float(c_account.account_value):
            raise ValueTooLargeException("Invalid withdraw amount (insufficient funds)")
        c_account.account_value -= withdraw
        # new_value = float(account.account_value) - withdraw
        # account.account_value = new_value
        return self.update_client_account(client_id, account_id, c_account)

    def deposit(self, client_id: int, account_id: int, deposit: float):
        if deposit < 0:
            raise NegativeNumberException("Invalid deposit amount (negative number)")
